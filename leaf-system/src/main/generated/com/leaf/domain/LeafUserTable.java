...
public class LeafUserTable extends AbstractTypedTable<LeafUser> implements LeafUserProps {
    ...
    public LeafUserTable createBy() {
        __beforeJoin();
        if (raw != null) {
            return new LeafUserTable(raw.joinImplementor(LeafUserProps.CREATE_BY.unwrap()));
        }
        return new LeafUserTable(joinOperation(LeafUserProps.CREATE_BY.unwrap()));
    }
    ...
    public LeafUserTable createBy(JoinType joinType) {
        __beforeJoin();
        if (raw != null) {
            return new LeafUserTable(raw.joinImplementor(LeafUserProps.CREATE_BY.unwrap(), joinType));
        }
        return new LeafUserTable(joinOperation(LeafUserProps.CREATE_BY.unwrap(), joinType));
    }

    @Override
    public PropExpression.Num<Long> createById() {
        return __getAssociatedId(LeafUserProps.CREATE_BY.unwrap());
    }
    ...
    public LeafUserTable updateBy() {
        __beforeJoin();
        if (raw != null) {
            return new LeafUserTable(raw.joinImplementor(LeafUserProps.UPDATE_BY.unwrap()));
        }
        return new LeafUserTable(joinOperation(LeafUserProps.UPDATE_BY.unwrap()));
    }
    ...
    public LeafUserTable updateBy(JoinType joinType) {
        __beforeJoin();
        if (raw != null) {
            return new LeafUserTable(raw.joinImplementor(LeafUserProps.UPDATE_BY.unwrap(), joinType));
        }
        return new LeafUserTable(joinOperation(LeafUserProps.UPDATE_BY.unwrap(), joinType));
    }

    @Override
    public PropExpression.Num<Long> updateById() {
        return __getAssociatedId(LeafUserProps.UPDATE_BY.unwrap());
    }
